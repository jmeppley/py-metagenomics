...
def countHits(infile, **kwargs):
    ...
    Calls edl.hits.parseM8FileIter with the following optional parameters:
        hitStringMap (None): dictionary (or file) mapping hit IDs to something else
        format (GENE): hit table format
        filterPct (0): only consider hits within this % of top score for each read
    ...
    if isinstance(taxonomy,str):
        taxonomy=readTaxonomy(taxonomy,namesMap=kwargs.pop('namesMap',False))
...
def countRefSeqHits(filename, **kwargs):
    """
    Counts hits from a hit table with defaults that make sense for a RefSeq search.


    Calls countHits with these defaults (that can be overriden):
        hitStringMap: /common/FASTA/NCBI/RefSeq/LATEST/acc.to.taxid.protein.plus
        taxonomy: read from /common/FASTA/NCBI/RefSeq/LATEST/taxdump

    You can also override the defaults with the parameter 'parseFilePrefix' which will replace '/common/FASTA/NCBI/RefSeq/LATEST/' in the defaults above.
    ...
    parseFilePrefix=kwargs.pop('parseFilePrefix','/common/FASTA/NCBI/RefSeq/LATEST/')
    kwargs.setdefault('hitStringMap',parseFilePrefix + 'acc.to.taxid.protein.plus')
