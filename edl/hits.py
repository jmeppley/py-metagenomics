...
def parseM8File(inhandle, hitStringMap, format, scorePct, parsingStyle, countMethod, taxonomy=None, rank=None, ignoreEmptyHits=True, sortReads=False):
    ...
    hitIter = parseM8FileIter(inhandle, hitStringMap, format, scorePct, parsingStyle, countMethod, taxonomy=taxonomy, rank=rank, ignoreEmptyHits=ignoreEmptyHits, sortReads=sortReads)
...
def parseHitsIter(hitIter, hitStringMap, parsingStyle, countMethod, taxonomy=None, rank=None, ignoreEmptyHits=None):
    """
...
def applyCountMethod(hitIter, method, ignoreEmpty=True):
    ...
    if ignoreEmpty:
        removeEmptyFunc=removeEmpty
    else:
        removeEmptyFunc=lambda x: x
...
def _findLeastCommonAncestor(hits):
    ...
    while i<len(hits):
        if hits[i] is None:
            hits.pop(i)
        elif type(hits[i]) is type(""):
            logger.info("Skipping hit: %s (cannot translate to taxon)" % (hits.pop(i)))
        else:
            i+=1
...
def parseHits(inhandle, readCol, hitCol, skipFirst, hitSep):
    ...
    """
    logger.debug("BEGIN parseHits(in, %r, %r, %r, %r)" % (readCol, hitCol, skipFirst, hitSep))
...
# the following functions take a line from a table and return a read name and an iterable collection of hits
def _getReadHitsEval(cells, readCol, hitCol, hitSep):
    """
    use eval to evaluate contents of hit cell. If resulting object is not iterable, put it into a tuple
    """
    read = cells[readCol]
    hit = cells[hitCol]
...
class HitTranslator:
    ...
    Return an object with translateHit method that will apply the mappings to a hit
    """
...
def getHitTranslator(hitStringMap=None, parseStyle=ORGS, taxonomy=None, rank=None, defaultToNone=True, hitsAreObjects=True):
    """
    Return a function that will return a list of organsims from a single hit.
        ...
        taxonomy (None): An edl.taxon.Taxonomy object or directory conatining taxdmp
    ...
    return HitTranslator(mappings,useDesc=infoInDescription,hitsAreObjects=hitsAreObjects)

# turn hit lines into organisms or KOs or anything else
def processHits(hitIter, **kwargs):
    """
    Take an in iterator over read,hits tuples and apply mappings using a HitTranslator
    """
...
def processHitsOld(hitIter, mapping=None, expr=None, taxIdMap=None, taxNameMap=None, defaultToNone=True, rank=None):
    """
    ...
    if defaultToNone is changed to False, anything not found in one of the mappings
    ...
    if logger.getEffectiveLevel() <= logging.DEBUG:
        ...
        logger.debug("Starting processHits:\n default to None: %r\n map: %s\n exp: %s\n taxa: %s\n rank: %s" % (defaultToNone, mapstr, exprstr, taxstr, rank))
...
# each function takes a hit and something else, and then reutrns a translated hit
