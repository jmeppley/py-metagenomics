...
def getApiKey(apiKeyFile='.galaxy_api_key', apiEnvVar='GALAXY_API_KEY'):
    ...
    elif os.path.exists(os.environ.get('HOME', '.') +
                        os.path.sep +
                        apiKeyFile):
        with open(os.environ.get('HOME', '.') +
                  os.path.sep +
                  apiKeyFile) as f:
            api_key = next(f).strip()
...
def findDatasets(apiKey, patterns,
                 ...
                 skipDeleted=True):
    """
    Return an iterator over (history,dataset) tuples where the history
    name matches one of the given patterns and the dataset is identified
    by name or number.
    """
    histories = set()
    for pattern in patterns:
        ...
        for history in getHistories(apiKey,
            ...
            if history[u'id'] in histories:
                logger.warn("Skipping history '%s(%s)', it was "
                            "processed in a previous regex" %
                            (history[u'name'], history[u'id']))
                continue
...
def getDatasetFile(apiKey, apiURL, historyName, datasetNumber,
   ...
   Uses getDatasetData() to find a given file. Only takes hitoryName
   and datasetNumber. Can also return just the URL (instead of an open
   file-like-objet)
    """
    for dataset in getDatasetData(apiKey, apiURL,
        ...
        raise Exception(
            "No match found for item %d in history '%s'" %
            (datasetNumber, historyName))
...
def getDatasetData(apiKey, apiURL,
    ...
    HistoryName can be a string or compiled re object. It will be ignored if
    historyId is not None.

    The same is not true for datasetNumbers and datasetName. All
    datasets mathing either will be returned.
    """
    ...
    for historyId in historyIds:
        ...
        for dataset in json.loads(urlopen(hurl).read()):
            ...
            hurl = apiURL + "/histories/" + historyId + \
                "/contents/" + dataset[u'id'] + "?key=" + apiKey

            details = json.loads(urlopen(hurl).read())
            if (details['deleted'] or details[
                    'state'] == 'error') and skipDeleted:
                continue
            if _dataset_match(details, datasetNumbers, datasetName):
                if 'download_url' not in details:
                    logger.warn(
                        "Can't find the download URL in %r\nHistory: %r" %
                        (details, historyId))
                    continue
                details['download_url'] = fixDownloadUrl(
                    details['download_url'], apiURL, apiKey)
                count += 1
                yield details
...
def _dataset_match(dataset, datasetNumbers, datasetName):
    ...
    if dataset[u'hid'] in datasetNumbers:
        ...
        return datasetName.search(dataset[u'name']) is not None
...
def getHistories(apiKey, apiURL,
    ...
    Optionally filter list with a regular expression (nameRE). If
    nameRE is a string, it must match exactly.
    """
...
def locateDatasets(
        runName,
        galaxyInstance,
        libraryNameTemplate="MiSeq Run: %s",
        sampleRE=None):
    """
    Parses a MiSeq or NextSeq run in the Galaxy shared library area and
    returns a dictionary keyed on the sample number (as a string, eg: '1').
    ...
    where the last two blanks are encoded Galaxy IDs for use in the API.
    MiSeq runs will only have the 001 lane, NextSeq runs should have up to 004.
    """
    files = {}
...
def loadSampleSheetFromGalaxy(sampleSheetId, galaxyInstance):
    ...
    """
    logging.debug(
        "Attempting to download SampleSheet %s from galaxy" %
        (sampleSheetId))
...
def parseSampleSheet(runName, sampleSheetId, galaxyInstance, **kwargs):
    """
    Parses a SampleSheet from the file system for the given run. The
    run name should correspond to a subdirectory of the 'dataDir'
    which defaults to /minilims/data/incoming/miseq.

    Returns a two-element tuple:
        chemistry: either 'nextera' or 'truseq'
        barcodes: dictionary from sample number (as integer) to
        two-element list of barcodes. Second element will be an empty
        string for truseq.
    """
    chemistry = 'scriptseq'
    barcodes = {}
    ...
    if sampleSheetId is None:
        ...
        f, closeStream = loadSampleSheetFromGalaxy(
            sampleSheetId, galaxyInstance)
...
def launchWorkflowOnSamples(
        apiKey,
        runName,
        workflowID=None,
        workflowName=None,
        historyPrefix='MGP.b011',
        apiURL=u'http://edminilims.mit.edu/api',
        chemistry=None,
        **kwargs):
    """
    Given:
    ...
    Runs the given workflow on each sample in the MiSeq run. The
    results are put into a new history with the name: "historyPrefix:
    runName: sampleName".
    """
    ...
    for (sample, sampleData) in files.iteritems():
        ...
        try:
            ...
            historyName = historyTemplate % (
                historyPrefix, runName, sampleName)
            for history in galaxyInstance.histories.get_histories(
                    name=historyName):
            ...
            if len(lanes) == 1:
                ...
                for (direction, inputid) in workflowInputs.iteritems():
                    if direction not in laneData:
                        raise Exception(
                            "Missing direction in input: %s" %
                            (direction))
                    dsMap[inputid] = {'src': 'ld', 'id': laneData[direction]}
            else:
                # NextSeq data has 4 lanes that need to be merged first
                historyId, datasets = launchNextSeqLaneMerge(
                    lanes, historyName, galaxyInstance, sampleName=sampleName)
...
def getPrimerToolId(galaxyInstance, workflowID):
    for stepid, step in galaxyInstance.workflows.show_workflow(workflowID)[
            u'steps'].iteritems():
...
def submit(api_key, url, data, return_formatted=True):
    """
    Sends an API POST request and acts as a generic formatter for
    the JSON response.
...
def findLibrariesWithName(
        name,
        galaxyInstance,
        libs_cache=None,
        createMissing=True):
    """
    Look for a library with the given name
    (on the server or in the given cached list).
    ...
    for lib in libs:
        if lib[u'name'] == name:
            lib[u'contents'] = galaxyInstance.libraries.show_library(
                lib[u'id'], contents=True)
    ...
    if len(libraries) == 0 and createMissing:
        ...
        if isinstance(lib, list):
            raise Exception(
                "create_library returned a list, this usually means your "
                "request was redirected and the library wasn't created. "
                "Try switching the root api url from http to https or "
                "vice versa")
        # Add class and contents to object to match what you get from
        # get_libraries()
...
def createOrFindFolderInLibrary(folder, library, galaxyInstance):
    ...
    First check the 'contents' entry, if missing, create folder in gi
    and add to contents
    Returns tuple with folder dict and boolean for whether or not it had
    to be created.
    The folder dict has 'library' entry added that points back to the
    enclosing library.
    """
    path = u'/' + folder
    ...
    for item in library[u'contents']:
        ...
        try:
            libid = library[u'id']
            data = galaxyInstance.libraries.create_folder(
                libid, folder, base_folder_id=rootFolder[u'id'])
        except Exception as inst:
            # This error usually happens after the folder is
            # created. Let's see if it exists...
            for item in galaxyInstance.libraries.show_library(
                    library[u'id'], contents=True):
                if u'type' in item and item[
                        u'type'] == u'folder' and item[u'name'] == path:
                    # found it!
...
def uploadFileToFolder(
    ...
    try:
        if metadata is not None:
            upload = galaxyInstance.libraries.upload_file_from_local_path(
                libID,
                fileName,
                folder_id=runFolderID,
                file_type=file_type,
                extended_metadata=metadata)
        else:
            # no metadata
            upload = galaxyInstance.libraries.upload_file_from_local_path(
                libID, fileName, folder_id=runFolderID, file_type=file_type)
        fileData = upload[0]
    except IncompleteRead as inst:
        # This error usually happens after the dataset is
        # created. Let's see if it exists:
        fileData = getFileDataFromLibrary(
            fullPath, galaxyInstance.libraries.        show_library(
                library[u'id']))
        if fileData is None:
            # Nope.
            raise inst
...
def getFileDataFromLibrary(fullPath, libraryContents):
    ...
    for item in libraryContents:
        if u'type' in item and item[
                u'type'] == u'file' and item[u'name'] in paths:
            return item
    return None
