...
translations = {
    'RoseobactercladeNAC11-7lineage': 'uncultured Roseobacter NAC11-7',
    ...
    def getCollapsedCounts2(self, counts, cutoff, collapsedCounts):
        ...
        if self in counts:
            ...
            for child in self.children:
                if child is self:
                    continue
                kidCount = child.getCollapsedCounts(
                    counts, cutoff, collapsedCounts)
...
def translateSSUTaxToNCBIDump(ssuFasta, ssuRanks, outDir):
    ...
    for (lineage, rank) in rankMap.iteritems():
        node = SilvaTaxNode.addToTreeFromString(
            lineage, silvaTree, removeSpaces=False)
        node.rank = rank
    ...
    with open(os.path.sep.join((outDir, 'ssu.hitid.to.taxid.silva')),
              'w') as taxMapFile:
