...
def crossTabulateHits(hitFile1,hitFile2,
    ...
    """
    Given two hit tables, cross reference reads to build a 2D DataFrame of hit counts
    """
    # get a map from reads to hits in file 1
    hitmap=tupleIteratorToMap(parseHits(hitFile1,readCol1,hitCol1,skipHeader1,hitSep1))
    ...
    # generate dataframe
    logger.warn("%d of %d reads only in file 1" % (len(hitmap),readMatchCount))
    logger.warn("%d of %d reads only in file 2" % (len(readsOnlyIn2),readMatchCount))
