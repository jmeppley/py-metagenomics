#! /usr/bin/python
"""
Library of methods and regular expressions for parsing files from the KEGG gene ontology
"""
...
def readSEEDTree(treeFile):
    """
    Return nested dictionary where first dict is map from levels (1,2,3) and next dict is map from role to name.
...
def parseSeedMap(mapFile):
    ...
    The SEED map (refseq2md52role.gz) starts with two summary lines followed by three columns (accession\thash/sum\tSubsystem):

    Mapped roles:9004(subsys.txt 2 subsystems2peg)
    Unmapped roles:521(s                ubsys.txt)
    YP_9218461b41910965945b806d5defc49ad1a224CO dehydrogenases       maturation factor, CoxF family
    YP_0012863261e472ed51c0df8feb03ee296a0  e55de4CO dehydrogenases maturation factor, CoxF family 
    """
    accMap={}
    with open(mapFile) as f:
        f.next()
...
def readKOFile(kofile, keggLevel):
    ...
    The level can be one of:
        NAME, PATHWAY, EC, DEFINITION, or a level in the CLASS heirachy: 1, 2, or 3
    ...
    if keggLevel in ['1','2','3',1,2,3]:
        ...
        logger.warn("KEGG leveles can not be parsed from ko files starting around 2013. This may fail unless you are using an older file")
        reString='CLASS'
    else:
        reString=keggLevel
    ...
    logger.info("Looking for %s with %s and (%s,%s)" % (repr(keggLevel), repr(nameRE.pattern), repr(nameSep), repr(nameIndex)))
    ...
    for line in open(kofile):
        ...
        # look for information on this KO
            ...
            if match:
                ...
                # contents of name line processed differently based on sep and index
...
def readKeggFile(keggFile, keggLevel):
    ...
    if keggLevel=='PATHWAY':
        keggLevel='3'
    kfile = open(keggFile)
    keggLevelMap = {'1':re.compile(r'^A\s*<[bB]>(.*)</[bB]>\s*$'),'2':re.compile(r'^B\s*<[bB]>(.*)</[bB]>\s*$'),'3':re.compile(r'^C\s*(\S.*)$')}
    ...
    if keggLevel=='NAME':
        ...
        sys.exit("Sorry, cannot currently parse EC from a .keg file, you'll need to use the 'ko' file")
    ...
    if pathRE!=None:
        ...
        for line in kfile:
            ...
            m = briteRE.search(line)
            if m:
                getDescriptionsFromBriteFile(m.group(1), keggFile, detailsRE, kmap)
                continue
...
def add_path_arguments(parser,defaults={},choices={},helps={}):
    ...
    pgroup.add_argument("-p", "--parseStyle",
                      ...
                      choices=[ACCS,GIS,KEGG,HITID,HITDESC,PFAM],
    ...
    if defaults.get("filterForPath",False):
        action='store_false'
        default=True
        help='Consider all hits. By deafult, only hits with path assignments are used.'
    else:
        action='store_true'
        default=False
...
def testReadKeggFile(keggFile):
    ...
    myAssertEq(k3map['K13810'],['01230 Biosynthesis of amino acids', '00010 Glycolysis / Gluconeogenesis', '00030 Pentose phosphate pathway', '00500 Starch and sucrose metabolism', '00520 Amino sugar and nucleotide sugar metabolism', '01000 Enzymes', '01000 Enzymes'])
