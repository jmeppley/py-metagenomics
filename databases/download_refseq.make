...
$(RSDIR)/complete/.download.complete.aa: $(RSDIR)/complete/complete.ftp.ls
	...
	ATTEMPTS=0; ERF=./.tmp.errs; CTF=./.tmp.cnts; for F in $$ERF $$CTF; do rm -f $$F; touch $$F; done; echo "start" > $$ERF
	while [ -s $$ERF ]; do echo starting attempt $$ATTEMPTS; for F in $$ERF $$CTF; do rm -f $$F; touch $$F; done; if [ "$$ATTEMPTS" -gt "0" ]; then echo Retrying RefSeq protein download; fi; if [ "$$ATTEMPTS" -gt "10" ]; then    echo ERROR: Exceeded 10 tries when downloading RefSeq; else cat RefSeq/78/complete/complete.ftp.ls | while read CPGZ; do  CPGZ_PATH=./RefSeq/78/complete/$${CPGZ};  if [ ! -s $$CPGZ_PATH ]; then   echo $$CPGZ_PATH>>$$CTF;   curl -s ftp://ftp.ncbi.nlm.nih.gov/refseq/release/complete/$${CPGZ} > $${CPGZ_PATH};  if [ ! -s $$CPGZ_PATH ]; then   echo $$CPGZ_PATH;    echo $$CPGZ_PATH>$$ERF;  fi; fi; done; echo downloaded `grep -c . $$CTF` files with `grep -c . $$ERF` errors; echo finished attempt $$ATTEMPTS; let ATTEMPTS=ATTEMPTS+1; fi; done
