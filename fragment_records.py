...
def main():
    ...
    parser.add_argument(
        ...
        dest="outname",
        metavar="OUTNAME",
        help="Create fragments with this name, except 'fragNNN' is "
             "inserted before any extension. Defaults to input file name "
             "(or './stdin')")
    parser.add_argument(
        "-Z",
        "--zero-padding",
        ...
        default=None,
        dest="padding",
        help="Zero pad fragment number in output file names. Leave blank "
             "to calculate from requested splits. Falls back to 5 if "
             "neither option used.")
    ...
    if arguments.infile is None and arguments.chunk is None:
        parser.error(
            "Cannot fragment STDIN unless a chunk size is specified. "
            "Please give an input file (-i) or chunk size (-C)")
    ...
    logging.debug(
        "outdir: '%s' :: outpref: '%s' :: outsuff: '%s'" %
        (outdir, outpref, outsuff))
