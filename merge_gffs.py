#!/usr/bin/env python
"""
Takes in a set of gff files in two lists. Features (like tRNA and rRNA) and then features that we don't want to overlap with them (like CDS from prodigal)

Outputs 3 files: a master gff, a matching fna of fastq seqs, and a faa of amino acid seqs of just the CDSs that don't overlap RNA.
"""
...
def get_regions(hits):
    ...
    regions=[]
    for hit in hits:
        # for our purposes, start is the lowest overlapped position, ignoring driection/strand
...
def merge_regions(regions, new_regions):
    """
    Simply combine lists for now. We way want to be more intelligent about htis in the future
    """
